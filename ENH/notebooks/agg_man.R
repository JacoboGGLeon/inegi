...
dunn_agg_man <- c()
range_agg_man <- c(3:10)
i <- 1

agg_man <- cluster::agnes(x = data, metric = "manhattan", method = "average")

for (h in range_agg_man) {
  set.seed(123)
  
  agg_man_group <- stats::cutree(tree = agg_man, k = h) 
  
  index_internal_agg_man <- clusterCrit::intCriteria(traj = data,
                                                     part = as.integer(agg_man_group), 
                                                     crit = "dunn")
  dunn_agg_man[i] <- index_internal_agg_man$dunn
  
  i <- i + 1
}

plot_dunn_agg_man <- ggplot2::qplot(x = range_agg_man, 
                                    y = dunn_agg_man, 
                                    geom = 'line', 
...
plot_dunn_agg_man + 
  theme_bw() +
  geom_vline(xintercept = range_agg_man[which.max(dunn_agg_man)], linetype = 2, color='red')

agg_man_group <- stats::cutree(tree = agg_man, k = range_agg_man[which.max(dunn_agg_man)]) 
